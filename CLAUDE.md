...
- **deploy-tier2.sh extra-vars format:** positional args extend to `sys.argv[22]` — `[11]`=`email_imap_host`, `[12]`=`email_imap_user`, `[13]`=`email_imap_password`, `[14]`=`email_imap_folder`, `[15]`=`reddit_enabled` (string `"true"`/`"false"`, converted to bool with `== 'true'`), `[16]`=`xurl_force_token` (string `"true"`/`"false"`, converted to bool), `[17]`=`gemini_key`, `[18]`=`openrouter_key`, `[19]`=`molt_api_key`, `[20]`=`molt_agent_name`, `[21]`=`obsidian_repo_slug`, `[22]`=`mistral_key`.
...
- **`bindings`** — Top-level array that routes inbound messages to specific agents by `{ channel, accountId }`. Required whenever multiple Telegram bot accounts are configured; without it every agent listens on every account. Format: `{ "agentId": "...", "match": { "channel": "telegram", "accountId": "..." } }`. A single-bot configuration needs no bindings array — the default agent receives all messages on that account.
