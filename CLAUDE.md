...
- **ThePincerMove digest agent (Phase 3 + 9f/9g/9k):** A second lightweight agent (`id: "pincermove"`, name: "ThePincerMove") always added alongside the main `RightClamp` agent. Pinned to `minimax/MiniMax-M2.1` (no fallbacks) to keep summarisation costs distinct in billing exports. Its workspace is `~/workspace-pincermove/` with minimal `AGENTS.md` and `SOUL.md` deployed from `roles/tier2-setup/files/workspace-pincermove/` (`force: false` — server edits survive re-deploy). Tool access is restricted: `write`, `edit`, `apply_patch`, `browser`, and `process` are denied; only `exec` (run scripts) and `read` remain. **Note:** The pincermove model is hardcoded to `minimax/MiniMax-M2.1` — if the LLM provider changes, the model must be updated in `openclaw.json.j2` or the agent will fail. All digest jobs always route to pincermove: substack digest (morning + evening), xurl timeline digest (morning + evening), reddit digest (morning + evening), daily stoic, `job-lastfm-sync`, `job-minimax-reminder`, and `job-openclaw-update-check`. The `job-feature-tips`, `job-feature-tips-monday`, `job-band-check`, and `job-obsidian-summarize` jobs stay on the default agent.
