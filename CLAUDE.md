...
- **Substack email digest (Phase 9f):** Gated on `email_imap_host | default('') | length > 0`. Deploys `check-substack-email.py`, `run-substack-check.sh`, and `substack-prompt.txt` directly to `~/scripts/` (not from a scripts repo). The cron job task runs unconditionally so it can also remove the job when email is later deconfigured — `SUBSTACK_ENABLED` env var controls enabled state. Delivery mode is `channel` when Telegram is configured, `none` otherwise. IMAP credentials land in `scripts-config.env` (mode 0600). State file at `~/.openclaw/email-state/processed.json` tracks processed Message-IDs across restarts. The Python script exits 0 with `[]` on any IMAP error so the cron job never fails due to outage. `python3` added to `exec-approvals.json.j2` so the agent can exec the poller via the bash wrapper. When migrating the prompt/scripts to the scripts repo, remove the three Phase 9f deploy tasks (the cron job task stays).
...
- `roles/tier2-setup/templates/check-substack-email.py.j2` — IMAP poller: detects new Substack emails, outputs JSON, tracks state
- `roles/tier2-setup/templates/run-substack-check.sh.j2` — Thin bash wrapper: sources scripts-config.env and execs the poller
- `roles/tier2-setup/templates/substack-prompt.txt.j2` — Agent instructions for summarising the poller output (temporary home; move to scripts repo later)
