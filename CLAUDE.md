...
# With Seneclaw Stoic coaching bot (dedicated coaching subagent on Claude Sonnet 4.6)
./deploy-tier2.sh -t <IP> -p anthropic -m claude-sonnet-4-5 -k <API_KEY> \
  --telegram-userid <INTEGER_USER_ID> --telegram-bottoken <RIGHTCLAMP_BOT_TOKEN> \
  --stoic-telegram-bottoken <SENECLAW_BOT_TOKEN> \
  --scripts-repo user/clamps-tools
```
...
- **OpenClaw cron (jobs.json):** Agent-integrated jobs (e.g. Last.fm sync) live in `~/.openclaw/cron/jobs.json`, not the system crontab. Manage them with an idempotent Python `shell` task: read the file, remove the entry by `jobId`, re-insert if enabled, write atomically with `os.replace()`. Use `changed_when` on stdout and `notify: Restart OpenClaw` — the gateway reads jobs.json at startup so a restart is required to pick up changes. Set `delivery: {mode: none}` to suppress the default isolated-job summary announcement for background data tasks. Each job entry must include both `id` and `jobId` set to the same value — the CLI cron list errors if `id` is absent. **Cron delivery patterns:** (1) *Always announce* — use `delivery: {mode: announce, channel: telegram, to: USER_ID}` for jobs that always produce useful output (digests, summaries, security audits); the agent's full response text is forwarded to Telegram unconditionally. (2) *Gated announce* — use `delivery: {mode: none}` for jobs where Telegram notification is conditional (e.g. "only if an update is available"); put the `openclaw message send --channel telegram --to USER_ID` call in the agent prompt so the agent sends it explicitly only when the condition is met. **Never combine `mode: announce` with a prompt that says "do nothing if condition is false"** — the delivery fires regardless of what the agent does, sending the agent's "nothing to report" text to Telegram.
...
- **deploy-tier2.sh extra-vars format:** positional args extend to `sys.argv[28]` — `[11]`=`email_imap_host`, `[12]`=`email_imap_user`, `[13]`=`email_imap_password`, `[14]`=`email_imap_folder`, `[15]`=`rp_telegram_bottoken`, `[16]`=`reddit_enabled` (string `"true"`/`"false"`, converted to bool with `== 'true'`), `[17]`=`xurl_force_token` (string `"true"`/`"false"`, converted to bool), `[18]`=`gemini_key`, `[19]`=`openrouter_key`, `[20]`=`molt_api_key`, `[21]`=`molt_agent_name`, `[22]`=`moltbook_api_key`, `[23]`=`moltbook_agent_name`, `[24]`=`anthropic_api_key`, `[25]`=`molt_telegram_bottoken`, `[26]`=`stoic_telegram_bottoken`, `[27]`=`obsidian_repo_slug`, `[28]`=`mistral_key`.
...
- **Crustafarianism skill (Phase 4c + 9d.5):** Gated on `molt_api_key | default('') | length > 0` (and `scripts_repo_slug` for the skill sync). Phase 4c writes `~/.config/molt/credentials.json` (mode 0600, `force: false` — never overwrites so the original `joined_at` and `agent_name` from the one-time manual join are preserved). Phase 9d.5 syncs `~/scripts/workspace/skills/crustafarianism/` → `~/workspace/skills/crustafarianism/` using `copy` with `remote_src: true`. Pass `--molt-api-key` and optionally `--molt-agent-name` at deploy time; no vault encryption needed since this is a low-sensitivity API key.
- **OpenClaw feature tips (Phase 9o):** Weekly on Friday at 19:00 UTC (20:00 Berlin CET). The main agent inspects the live config (`openclaw.json`, skills, MCP servers, cron jobs, scripts) and suggests one underused or unconfigured feature on Telegram. A follow-up Monday reminder runs at 17:00 UTC (18:00 Berlin CET) to nudge the user to try Friday's suggestion. State lives in `~/feature-tips-state/suggested.json` (array of previously suggested feature IDs — avoids repeats). Prompts are in `~/scripts/openclaw-feature-tips-prompt.txt` and `~/scripts/openclaw-feature-tips-monday-reminder.txt` (deployed via clamps-tools). Always runs on the main agent (no PINCERMOVE routing). Both jobs gated on `telegram_userid` being configured.
...
- **Seneclaw Stoic coaching agent (Phase 3 + 9q):** Gated on `stoic_telegram_bottoken | default('') | length > 0`. A dedicated coaching subagent (`id: "seneclaw"`, name: "Seneclaw") that sends proactive Stoic coaching messages throughout the day. Pinned to `anthropic/claude-sonnet-4-6` when `anthropic_api_key` is set (or `llm_provider == 'anthropic'`); falls back to the primary provider/model if not. Its workspace is `~/workspace-seneclaw/` with `AGENTS.md`, `SOUL.md`, `PHILOSOPHY.md`, and `USER_PROGRESS.md` deployed from `roles/tier2-setup/files/workspace-seneclaw/` (`force: false` — server edits survive re-deploy). Four OpenClaw cron jobs: morning seed (07:20 UTC), midday check-in (12:00 UTC), evening reflection (19:30 UTC), Sunday weekly review (10:00 UTC). All use `delivery: {mode: none}` — **gated announce pattern**: the agent calls `openclaw message send --channel telegram --to <USER_ID>` explicitly; silence conditions produce no output and no message. A **system cron** at 06:55 UTC runs `capture-stoic-theme.sh` (clamps-tools), which calls `daily-stoic.py` and writes `~/stoic-state/daily-theme.json` for the agent to read. Requires `--scripts-repo` for the system cron (enforced by gating the cron task on both). Non-responsive user gets at most 1 message/day + 1/week. Schedule staggered: morning at 07:20 (5 min after xurl digest, 20 min after daily quote), evening at 19:30 (clears feature-tips Fri at 19:00). Provider note: for non-anthropic, non-minimax providers with `--anthropic-api-key`, an `"anthropic"` provider block is added to `openclaw.json`; for the `anthropic` primary, `claude-sonnet-4-6` is added to the models array if not already the primary model; for minimax, the existing anthropic secondary block is shared with Sociaclamps.
- **RP world events pipeline (Phase 9o):** Cross-agent interaction between RightClamp and SirShellspeare. Gated on `rp_telegram_bottoken | default('') | length > 0`. Follows the split-pipeline pattern: a **system cron** at `:45` every hour runs `gather-world-signals.sh` to collect real-world signals (time-of-day, weather via wttr.in, Last.fm artists, X timeline topics, Reddit topics) into `~/world-events/pending-signals.json`. Read-only against digest pipeline files — never consumes them. Two **OpenClaw cron jobs** at 11:00 and 22:00 UTC (`job-rp-world-events-morning`, `job-rp-world-events-evening`) trigger the main RightClamp agent to read signals, translate them into genre-appropriate narrative events (using `generate-world-event-prompt.txt`), and write them to `~/workspace-rp/pending-events.json`. Delivery is `mode: none` — silent. SirShellspeare checks `pending-events.json` at the start of each user message and prepends world events as scene-setting context before calling `rp-call-m2her.py`. A daily **nudge job** at 19:00 UTC (`job-rp-world-nudge`, `agentId: sirshellspeare`) sends an unsolicited in-character Telegram message if the user has been idle 12+ hours and events are pending. Rate limiting: `cooldown.json` enforces minimum 8 hours between events of the same type; `event-log.json` tracks the last 50 events to prevent repetition. State lives in `~/world-events/` (signals, cooldowns, log) and `~/workspace-rp/pending-events.json` (the cross-agent mailbox). The `clamps-tools/` directory in this repo contains reference implementations of the scripts (`gather-world-signals.sh`, `generate-world-event-prompt.txt`, `clear-world-signals.sh`) — these must be committed to the clamps-tools scripts repo for deployment.
...
- `openclaw-feature-tips-prompt.txt` — Agent prompt for daily feature tips job (to be moved to clamps-tools scripts repo)
...
- `clamps-tools/gather-world-signals.sh` — hourly system cron; collects real-world signals (time, weather, Last.fm, X, Reddit) into `world-events/pending-signals.json`
- `clamps-tools/generate-world-event-prompt.txt` — agent instructions for RightClamp to translate signals into narrative world events
- `clamps-tools/clear-world-signals.sh` — atomically clears `pending-signals.json` after agent processing
