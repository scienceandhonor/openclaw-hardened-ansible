...
- **OpenClaw feature tips (Phase 9o):** Daily at 19:00 UTC (20:00 Berlin CET). The main agent inspects the live config (`openclaw.json`, skills, MCP servers, cron jobs, scripts) and suggests one underused or unconfigured feature on Telegram. State lives in `~/feature-tips-state/suggested.json` (array of previously suggested feature IDs — avoids repeats). Prompt is in `~/scripts/openclaw-feature-tips-prompt.txt` (source: `openclaw-feature-tips-prompt.txt` in the Ansible repo root, deployed manually to clamps-tools). Always runs on the main agent (no PINCERMOVE routing). Gated on `telegram_userid` being configured.
