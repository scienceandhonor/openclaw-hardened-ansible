...
- **`exec-approvals.json` (Tier 2):** Controls shell command execution approval per agent. Deployed from `roles/tier2-setup/templates/exec-approvals.json.j2` to `~/.openclaw/exec-approvals.json` on the target host. Each agent can be set to `allowlist` (whitelist mode, `ask: off` or `ask: on-miss`), `deny` (block all exec), or inherit `defaults`. The `main` agent uses `security: deny, ask: always` so interactive exec always requires approval. The `pincermove` agent uses `security: allowlist, ask: off` with a comprehensive allowlist of all scripts and binaries its cron jobs need — this prevents approval prompts on all recurring background jobs. The `sirshellspeare` agent also uses `allowlist, ask: off`. When adding a new script to a cron job's prompt, add its path to the appropriate agent's allowlist in `exec-approvals.json.j2` so the agent can run it without triggering an approval request. The `openclaw_bin` path is templated as `{{ openclaw_bin.stdout }}` so it resolves correctly on the remote host.
...
- **deploy-tier2.sh extra-vars format:** positional args extend to `sys.argv[23]` — `[11]`=`email_imap_host`, `[12]`=`email_imap_user`, `[13]`=`email_imap_password`, `[14]`=`email_imap_folder`, `[15]`=`rp_telegram_bottoken`, `[16]`=`reddit_enabled` (string `"true"`/`"false"`, converted to bool with `== 'true'`), `[17]`=`xurl_force_token` (string `"true"`/`"false"`, converted to bool), `[18]`=`gemini_key`, `[19]`=`openrouter_key`, `[20]`=`molt_api_key`, `[21]`=`molt_agent_name`, `[22]`=`obsidian_repo_slug`, `[23]`=`mistral_key`.
...
- **OpenClaw feature tips (Phase 9o):** Weekly on Friday at 19:00 UTC (20:00 Berlin CET). The main agent inspects the live config (`openclaw.json`, skills, MCP servers, cron jobs, scripts) and suggests one underused or unconfigured feature on Telegram. A follow-up Monday reminder runs at 17:00 UTC (18:00 Berlin CET) to nudge the user to try Friday's suggestion. State lives in `~/feature-tips-state/suggested.json` (array of previously suggested feature IDs — avoids repeats). Prompts are in `~/scripts/openclaw-feature-tips-prompt.txt` and `~/scripts/openclaw-feature-tips-monday-reminder.txt` (deployed via clamps-tools). Both jobs run on `pincermove` (not the main agent). Both gated on `telegram_userid` being configured.
...
- **ThePincerMove digest agent (Phase 3 + 9f/9g/9k):** A second lightweight agent (`id: "pincermove"`, name: "ThePincerMove") always added alongside the main `RightClamp` agent. Primary model `minimax/MiniMax-M2.7` with `minimax/MiniMax-M2.1` as fallback — keeps summarisation costs distinct in billing exports while benefiting from M2.7 quality. Its workspace is `~/workspace-pincermove/` with minimal `AGENTS.md` and `SOUL.md` deployed from `roles/tier2-setup/files/workspace-pincermove/` (`force: false` — server edits survive re-deploy). Tool access is restricted: `write`, `edit`, `apply_patch`, `browser`, and `process` are denied; only `exec` (run scripts) and `read` remain. All cron jobs route to pincermove: substack digest (morning + evening), xurl timeline digest (morning + evening), reddit digest (morning + evening), daily stoic, `job-lastfm-sync`, `job-minimax-reminder`, `job-openclaw-update-check`, `job-band-check`, `job-feature-tips`, `job-feature-tips-monday`, and `job-obsidian-summarize`. The `job-rp-world-events-morning`, `job-rp-world-events-evening`, and `job-rp-world-nudge` jobs stay on the default agent (RightClamp) and SirShellspeare respectively.
